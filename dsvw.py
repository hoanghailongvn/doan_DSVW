...
# Configuration
STATIC_FOLDER = "static"  # Folder containing static files for serving
        ...
        try:
            if path == '/':
                ...
                if HTML_PREFIX in content and HTML_POSTFIX not in content:
                    content += "<div><span>Attacks:</span></div>\n<ul>%s\n</ul>\n" % ("".join("\n<li%s>%s - <a href=\"%s\">vulnerable</a></li>" % (" class=\"disabled\" title=\"module 'python-lxml' not installed\"" if ("lxml.etree" not in sys.modules and any(_ in case[0].upper() for _ in ("XML", "XPATH"))) else "", case[0], case[1]) for case in CASES)).replace("<a href=\"None\">vulnerable</a>|", "<b>-</b>|")
              ...
              path = os.path.normpath(os.path.join(STATIC_FOLDER, self.path[len(STATIC_URL_PATH):].lstrip('/')))
